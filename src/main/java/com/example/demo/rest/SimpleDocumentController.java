 ...
 * <p>
 * produces = "!application/jose" писать необязательно, т.к. нет никакого криминала в том, чтобы вернуть пользователю
 * (конечно если он это запросит с помощью заголовка Accept) ответ с нашей транспортной подписью
 */
@RestController
